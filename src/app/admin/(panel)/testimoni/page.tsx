...
import { PenSquare, Plus, Star, Trash2, Loader2, Search, CheckCircle, XCircle } from 'lucide-react';
...
export default function KelolaTestimoni() {
  ...
  return (
    <>
      <AdminHeader title="Kelola Testimoni">
        <Link 
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white p-4 rounded-xl border border-gray-100 shadow-xs">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              type="text"
              placeholder="Cari nama atau kota jamaah..."
              value={search}
            ...
            <table className="admin-table w-full text-left text-sm text-gray-500">
              <thead className="bg-gray-50/80 text-xs text-gray-700 uppercase font-semibold border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-4">Teks Testimoni</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-gray-400">
                      <p className="text-base font-medium text-gray-600 mb-1">Belum ada testimoni</p>
                      <p className="text-xs">Klik tombol &ldquo;Tambah Testimoni&rdquo; untuk menambahkan ulasan jamaah.</p>
                    </td>
                  </tr>
                ) : (
                  filtered.map((testimoni) => (
                    <tr key={testimoni.id} className="hover:bg-gray-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center text-gold-500">
                          {[...Array(5)].map((_, i) => (
                            <Star 
                              key={i} 
                              className={`w-4 h-4 ${i < testimoni.rating ? 'fill-current text-[#C9A227]' : 'text-gray-200'}`} 
                            />
                          ))}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-1.5">
                          <Link 
                            href={`/admin/testimoni/edit/${testimoni.id}`} 
                          ...
                          <button
                            type="button"
                            onClick={() => setDeleteId(testimoni.id)}
